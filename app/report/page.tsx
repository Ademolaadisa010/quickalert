...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes countdownShrink{from{stroke-dashoffset:0}to{stroke-dashoffset:283}}
    ...
    navigator.geolocation.getCurrentPosition(
      async pos => {
        ...
        try {
          ...
          setGps(g=>({...g,address:d.display_name?.split(",").slice(0,3).join(", ")||`${lat.toFixed(4)}° N, ${lng.toFixed(4)}° E`}));
...
function useVideoCapture() {
  ...
  const [recStatus,   setRecStatus]   = useState<"idle"|"recording"|"done">("idle");
  const [facingMode,  setFacingMode]  = useState<"environment"|"user">("environment");
  const [elapsed,     setElapsed]     = useState(0);       // 0–10
  ...
  const startRecording = useCallback(() => {
    if (!streamRef.current || recStatus==="recording") return;
    chunksRef.current = [];
    setElapsed(0);
    setVideoBlob(null);
    setVideoUrl(null);
...
export default function ReportPage() {
  ...
  // Once video is done → go to verify
  useEffect(() => {
        ...
        <main className="qa-main">
          ...
          <div style={{padding:"20px",maxWidth:560,margin:"0 auto"}}>
            ...
            {cam.camStatus === "error" ? (
              ...
              /* Video is done — show playback + confirm */
              cam.videoUrl ? (
                <div>
                  ...
                  <div className="cam-wrap" style={{marginBottom:14}}>
                    {cam.camStatus === "active" ? (
                      <>
                        ...
                        {/* Countdown ring (while recording) OR plain record button (idle) */}
                        {cam.recStatus === "recording" ? (
        ...
        <main className="qa-main">
          ...
          <div style={{padding:"24px 20px",maxWidth:520,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {[["Video analysis","Scene matches reported type"],["GPS verification","Location confirmed on grid"],["Pattern analysis","Movement data analysed"]].map(([label,value],i) => (
      ...
      <div className="qa-shell">
        ...
        <main className="qa-main">
          ...
          <div style={{padding:"24px 20px",maxWidth:600,margin:"0 auto"}}>
            ...
            {/* Video CTA tile */}
            <button
              style={{width:"100%",background:"var(--s2)",border:`1px dashed ${type?"rgba(230,57,70,.35)":"var(--b1)"}`,borderRadius:14,height:100,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6,cursor:type?"pointer":"not-allowed",opacity:type?1:.5,marginBottom:14,transition:"all .15s"}}
