...
/* ── Camera hook (recStatusRef fix included) ── */
...
async function uploadVideo(blob:Blob):Promise<{url:string;publicId:string}|null>{
  try{
    ...
    const resp=await fetch("/api/upload",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({data:dataUrl,resourceType:"video"})});
...
type AIResult={
  ...
  whatISee:string;
  summary:string;
  checks:{imageAnalysis:{passed:boolean;detail:string};gpsConsistency:{passed:boolean;detail:string};patternMatch:{passed:boolean;detail:string}};
};

async function runAIAnalysis(frameDataUrl:string,incidentType:string,gpsLat:number|null,gpsLng:number|null,gpsAccuracy:number|null,notes:string):Promise<AIResult|null>{
  try{
    const resp=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({frameDataUrl,incidentType,gpsLat,gpsLng,gpsAccuracy,notes})});
    const json=await resp.json();
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   This is the key function that connects report → dashboard.
   It writes a document to the "incidents" collection which the dashboard
   reads via its onSnapshot listener.
══════════════════════════════════════════════════════════════════════════ */
async function saveIncidentToFirestore(data: {
  type:        string;
  loc:         string;
  lat:         number | null;
  lng:         number | null;
  ...
  try {
    const priority = data.conf >= 75 ? "high" : data.conf >= 50 ? "medium" : "low";
    const docRef = await addDoc(collection(db, "incidents"), {
      type:       data.type,
      loc:        data.loc || "Location not available",
      city:       "Ibadan",
      lat:        data.lat    ?? 7.3776,
      lng:        data.lng    ?? 3.9470,
      conf:       data.conf,
...
export default function ReportPage() {
  ...
  // const [aiVerdict,    setAiVerdict]    = useState<"YES"|"NO"|"UNKNOWN"|"">("");
  // const [aiWhatISee,   setAiWhatISee]   = useState("");
  const [aiSummary,    setAiSummary]    = useState("");
  const [aiLevel,      setAiLevel]      = useState<"LOW"|"MEDIUM"|"HIGH"|"">("");
  const [aiVerdict,    setAiVerdict]    = useState<"YES"|"NO"|"UNKNOWN"|"">("");
  const [aiWhatISee,   setAiWhatISee]   = useState("");
  ...
  useEffect(()=>{if(step===1)cam.startCamera("environment");},[step]); // eslint-disable-line
  useEffect(()=>{if(cam.videoBlob&&step===1)startVerify();},[cam.videoBlob]); // eslint-disable-line

  const startVerify = async () => {
    if(!cam.videoBlob)return;
    setStep(2);
    setConf(0);setChecks([false,false,false]);setCheckDetails(["Processing…","Processing…","Processing…"]);
    setUploadProg(0);setUploadErr("");setAiSummary("");setAiLevel("");setSaveErr("");setFirestoreId(null);

    let frameDataUrl="";
    try{frameDataUrl=await extractFrame(cam.videoBlob);}catch{}
    ...
    const[uploadResult,aiResult]=await Promise.all([
      ...
      frameDataUrl?runAIAnalysis(frameDataUrl,type,gps.lat,gps.lng,gps.accuracy,notes):Promise.resolve(null),
    ]);

    clearInterval(progTimer);
    setUploadProg(100);

    const mediaUrl = uploadResult?.url ?? null;
    if(uploadResult)setUploadedUrl(mediaUrl);
    else setUploadErr("Upload failed — report includes local video.");

    const finalConf    = aiResult?.confidence ?? 72;
    const finalLevel   = aiResult?.level      ?? "MEDIUM";
    const finalSummary = aiResult?.summary    ?? "Alert verified and ready to dispatch.";
    const finalFake    = aiResult?.fake       ?? false;
    const finalChecks  = aiResult
      ? [aiResult.checks.imageAnalysis.passed, aiResult.checks.gpsConsistency.passed, aiResult.checks.patternMatch.passed]
      : [true, !!gps.lat, true];
    const finalDetails = aiResult
      ? [aiResult.checks.imageAnalysis.detail, aiResult.checks.gpsConsistency.detail, aiResult.checks.patternMatch.detail]
      : ["Scene analysed","GPS location recorded","Pattern matched"];

    setAiSummary(finalSummary);
    setAiLevel(finalLevel);
    setAiVerdict(aiResult?.verdict ?? "UNKNOWN");
    setAiWhatISee(aiResult?.whatISee ?? "");
    setAiSummary(finalSummary);
    setAiLevel(finalLevel);
    setAiVerdict(aiResult?.verdict ?? "UNKNOWN");
    setAiWhatISee(aiResult?.whatISee ?? "");

    // ── SAVE TO FIRESTORE (happens in background while bar animates) ──────
    saveIncidentToFirestore({
      ...
      if(id)setFirestoreId(id);
      else setSaveErr("Could not save to database. Check Firestore rules.");
    });

    // Animate confidence bar
    let v=0;
    const STEP_MS=40;
    const inc=finalConf/(2200/STEP_MS);
    ...
    animTimer.current=setInterval(()=>{
      ...
      if(pct>=0.85){setChecks(finalChecks);setCheckDetails(finalDetails);}
      if(v>=finalConf){
        clearInterval(animTimer.current!);
        setChecks(finalChecks);setCheckDetails(finalDetails);
  ...
  const reset=()=>{
    ...
    setAiVerdict("");setAiWhatISee("");setFirestoreId(null);setSaveErr("");
    setUploadProg(0);setUploadedUrl(null);setUploadErr("");setAiSummary("");setAiLevel("");
    setAiVerdict("");setAiWhatISee("");setFirestoreId(null);setSaveErr("");
  ...
  if(step===1)return(
    ...
    <div className="qa-shell"><Sidebar path={path}/>
      <main className="qa-main"><Header/>
        <div style={{padding:"20px",maxWidth:560,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
            <button onClick={()=>setStep(0)} style={{background:"none",border:"none",color:"var(--muted)",cursor:"pointer",display:"flex",alignItems:"center",gap:5,fontSize:13}}>
              <svg viewBox="0 0 20 20" fill="currentColor" width="16" height="16"><path fillRule="evenodd" d="M17 10a.75.75 0 01-.75.75H5.612l4.158 3.96a.75.75 0 11-1.04 1.08l-5.5-5.25a.75.75 0 010-1.08l5.5-5.25a.75.75 0 111.04 1.08L5.612 9.25H16.25A.75.75 0 0117 10z" clipRule="evenodd"/></svg>Back
            </button>
            <div><p className="lbl" style={{color:"var(--red)"}}>10-second live video</p><h2 style={{fontSize:18,fontWeight:700}}>Record the scene</h2></div>
          </div>

          {cam.camStatus==="error"?(
            <div style={{background:"var(--s2)",border:"1px solid rgba(230,57,70,.25)",borderRadius:16,padding:"28px 20px",textAlign:"center"}}>
              ...
              <div className="cam-wrap" style={{marginBottom:14}}>
                {cam.camStatus==="active"?(
                  <>
                    ...
                    {cam.recStatus==="recording"&&<div className="rec-badge"><div className="rec-dot"/>REC &nbsp;{cam.elapsed}s / {MAX_SECONDS}s</div>}
                    {gps.status==="locked"&&<div className="gps-overlay">📍 {gps.lat?.toFixed(4)}°N {gps.lng?.toFixed(4)}°E</div>}
                    ...
                    {cam.recStatus==="recording"?(
                      ...
                      <div className="rec-btn-wrap">
                        <button className="rec-btn" onClick={cam.startRecording}><div className="rec-inner"/></button>
                      </div>
                    )}
                  </>
                ):(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:10,color:"var(--muted)",padding:40}}>
                    <div className="spin-ring" style={{width:28,height:28,borderWidth:3}}/><span style={{fontSize:12}}>Starting camera…</span>
                  </div>
                )}
              </div>
      ...
      <main className="qa-main"><Header/>
        <div style={{padding:"24px 20px",maxWidth:520,margin:"0 auto"}}>
              ...
              <div style={{padding:"10px 14px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12}}>
                  <span style={{fontFamily:"'DM Mono',monospace",color:"var(--muted)"}}>
                    {uploadProg<100?`Uploading… ${uploadProg}%`:uploadedUrl?"✓ Video secured on Cloudinary":uploadErr||"Uploading…"}
                  </span>
                ...
                <div className="upload-bar"><div className="upload-fill" style={{width:`${uploadProg}%`,background:uploadErr?"var(--amber)":"var(--green)"}}/></div>
              </div>
            </div>
          )}
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:14,textAlign:"center",marginBottom:24}}>
            ...
            <div>
              <p className="lbl" style={{color:"var(--amber)",marginBottom:8}}>Gemini AI · Verification</p>
              <h2 style={{fontSize:20,fontWeight:700,marginBottom:4}}>Verifying alert…</h2>
              <p style={{color:"var(--muted)",fontSize:13,lineHeight:1.6}}>Analysing video frame, GPS consistency<br/>and saving to Firestore</p>
            </div>
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {[["Video analysis",checkDetails[0]],["GPS verification",checkDetails[1]],["Pattern analysis",checkDetails[2]]].map(([label,detail],i)=>(
          ...
          {/* ── Gemini verdict card ─────────────────────────────── */}
          {aiVerdict && (
            <div style={{marginTop:16,animation:"fadeIn .35s ease both"}}>

              {/* YES / NO big verdict */}
              <div style={{
                background: aiVerdict==="YES" ? "rgba(230,57,70,.1)" : aiVerdict==="NO" ? "rgba(82,183,136,.1)" : "rgba(255,255,255,.05)",
                border: `1px solid ${aiVerdict==="YES"?"rgba(230,57,70,.3)":aiVerdict==="NO"?"rgba(82,183,136,.3)":"rgba(255,255,255,.1)"}`,
                borderRadius:12, padding:"14px 16px", marginBottom:10
              }}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                  <div style={{
                    fontSize:22, fontWeight:700, fontFamily:"'DM Mono',monospace",
                    color: aiVerdict==="YES"?"var(--red)":aiVerdict==="NO"?"var(--green)":"var(--amber)"
                  }}>
                    {aiVerdict==="YES"?"⚠ YES":"✓ NO"}{aiVerdict==="UNKNOWN"?" UNCLEAR":""}
                  </div>
                  <div style={{fontSize:11,color:"var(--muted)"}}>
                    {aiVerdict==="YES"
                      ? `Emergency confirmed — ${conf}% confidence`
                      : aiVerdict==="NO"
                        ? `No emergency detected — ${100-conf}% confidence it's clear`
                        : "Inconclusive — manual review needed"}
                  </div>
                  <div style={{
                    marginLeft:"auto", fontFamily:"'DM Mono',monospace", fontSize:10,
                    padding:"3px 10px", borderRadius:99,
                    background: aiLevel==="HIGH"?"var(--green-dim)":aiLevel==="MEDIUM"?"var(--amber-dim)":"var(--red-dim)",
                    color: aiLevel==="HIGH"?"var(--green)":aiLevel==="MEDIUM"?"var(--amber)":"var(--red)",
                    border: `1px solid ${aiLevel==="HIGH"?"rgba(82,183,136,.25)":aiLevel==="MEDIUM"?"rgba(244,162,97,.25)":"rgba(230,57,70,.25)"}`
                  }}>{aiLevel}</div>
                </div>
                {/* What Gemini sees */}
                {aiWhatISee && (
                  <div style={{fontSize:12,color:"var(--muted)",lineHeight:1.6,padding:"8px 10px",background:"rgba(0,0,0,.25)",borderRadius:8}}>
                    <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(96,165,250,.7)",marginRight:6}}>Gemini sees →</span>
                    {aiWhatISee}
                  </div>
                )}
              </div>
              ...
              {/* Summary */}
              {aiSummary && (
                <div style={{fontSize:12,color:"var(--muted)",lineHeight:1.6,padding:"10px 14px",background:"rgba(255,255,255,.03)",borderRadius:8,border:"1px solid var(--b1)"}}>
                  <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(96,165,250,.6)",display:"block",marginBottom:4}}>AI Assessment</span>
                  {aiSummary}
                </div>
              )}
            </div>
        ...
        <div style={{padding:"24px 20px",maxWidth:520,margin:"0 auto"}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",marginBottom:28,gap:14}}>
            ...
            <div>
              ...
              <p style={{fontFamily:"'DM Mono',monospace",color:"var(--green)",fontSize:13}}>Confidence {conf}% · {aiLevel||"MEDIUM"} · ETA ~4 min</p>
            </div>
          </div>

          {/* Firestore save status */}
          {firestoreId&&(
            <div style={{marginBottom:14,background:"var(--green-dim)",border:"1px solid rgba(82,183,136,.25)",borderRadius:10,padding:"10px 14px",display:"flex",alignItems:"center",gap:8,fontSize:12}}>
              <span style={{color:"var(--green)"}}>✓ Saved to database</span>
              <span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(82,183,136,.6)",marginLeft:"auto"}}>ID: {firestoreId.slice(0,8)}…</span>
            </div>
          )}
          {saveErr&&(
            <div style={{marginBottom:14,background:"var(--red-dim)",border:"1px solid rgba(230,57,70,.25)",borderRadius:10,padding:"10px 14px",fontSize:12,color:"var(--red)"}}>
              ⚠ {saveErr}
            </div>
          )}
          ...
          <div className="card fadeUp" style={{padding:"18px 20px",marginBottom:18}}>
            ...
            {[
              ...
              ["Location",    gps.address||"Not available"],
              ...
              ["Firestore",   firestoreId?"Saved ✓":saveErr?"Failed":"Saving…"],
              ["Dispatched to","Nearest verified hospital"],
            ].map(([k,v])=>(
              <div key={k} className="summary-row">
                <span style={{color:"var(--muted)"}}>{k}</span>
                <span style={{color:["AI confidence","Video","Firestore"].includes(k)&&(uploadedUrl||firestoreId)?"var(--green)":"#fff",fontFamily:"'DM Mono',monospace",fontSize:12}}>{v}</span>
              </div>
            ))}
          </div>
