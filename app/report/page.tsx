...
/* ─────────────── find nearest approved hospital from Firestore ──────────── */
async function findNearestHospital(lat:number|null,lng:number|null):Promise<{uid:string;name:string;hospital:string;email:string;phone:string;distKm:number}|null>{
  if(!lat||!lng)return null;
  try{
    const snap=await getDocs(query(collection(db,"responders"),where("status","==","approved")));
    const candidates=snap.docs
      .map(d=>{
        const data=d.data() as any;
        // Only include if they have lat/lng stored OR we match by city (fallback)
        return{uid:d.id,...data};
      })
      .filter(r=>r.hospitalLat&&r.hospitalLng); // hospitals that stored their GPS

    if(candidates.length===0){
      // Fallback: just return first approved hospital without GPS filtering
      const first=snap.docs[0];
      if(!first)return null;
      const data=first.data() as any;
      return{uid:first.id,name:data.name,hospital:data.hospital,email:data.email,phone:data.phone,distKm:0};
    }

    // Sort by distance and pick nearest
    const sorted=candidates
      .map(r=>({...r,distKm:haversine(lat,lng,r.hospitalLat,r.hospitalLng)}))
      .sort((a,b)=>a.distKm-b.distKm);

    const best=sorted[0];
    return{uid:best.uid,name:best.name,hospital:best.hospital,email:best.email,phone:best.phone,distKm:Math.round(best.distKm*10)/10};
  }catch(e){
    console.error("[findNearestHospital]",e);
    return null;
  }
}

/* ─────────────── save incident + dispatch to nearest hospital ───────────── */
async function saveAndDispatch(data:{
  type:string;loc:string;lat:number|null;lng:number|null;accuracy:number|null;
  conf:number;level:string;mediaUrl:string|null;anonymous:boolean;notes:string;
  aiSummary:string;fake:boolean;
}):Promise<{incidentId:string|null;hospital:string;distKm:number}>{

  // 1. Find nearest hospital
  const nearest=await findNearestHospital(data.lat,data.lng);
  const hospitalName = nearest?.hospital ?? "Nearest verified hospital";
  const distKm       = nearest?.distKm   ?? 0;
  const dispatchedTo = nearest?.uid      ?? null;
  ...
  // 3. Write a dispatch notification to the hospital's responder doc
  if(nearest?.uid&&incidentId){
    try{
      await setDoc(
        doc(db,"dispatches",incidentId),
        {
          incidentId,
          hospitalUid:  nearest.uid,
      ...
      console.log("✅ Dispatch sent to:",nearest.hospital,"("+distKm+"km)");
    }catch(e){
      console.error("❌ Dispatch write error:",e);
    }
  }

  return{incidentId,hospital:hospitalName,distKm};
}
...
export default function ReportPage() {
  ...
  const [dispatchedTo,   setDispatchedTo]   = useState("Nearest verified hospital");
  const [dispatchDist,   setDispatchDist]   = useState<number|null>(null);
  ...
  const startVerify = async () => {
    ...
    // 4) Save to Firestore + dispatch to nearest hospital (background)
    saveAndDispatch({
      type,
      loc:      gps.address,
      lat:      gps.lat,
      ...
      setDispatchedTo(result.hospital);
      if(result.distKm>0)setDispatchDist(result.distKm);
    });
  ...
  const reset = () => {
    ...
    setDispatchedTo("Nearest verified hospital");
    setDispatchDist(null);
    setIncidentId(null);
          ...
          <div style={{padding:"24px 20px",maxWidth:520,margin:"0 auto"}}>
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",marginBottom:28,gap:14}}>
              ...
              <div>
                ...
                <p style={{color:"var(--muted)",fontSize:14,marginBottom:4}}>
                  Alert dispatched to <strong style={{color:"#fff"}}>{dispatchedTo}</strong>
                </p>
                <p style={{fontFamily:"'DM Mono',monospace",color:"var(--green)",fontSize:13}}>
                  {conf}% confidence · {aiLevel||"MEDIUM"}{dispatchDist?` · ${dispatchDist}km away`:""}
                </p>
              </div>
            </div>
            ...
            <div className="card fadeUp" style={{padding:"18px 20px",marginBottom:18}}>
              ...
              {[
                ...
                ["Dispatched to",  `${dispatchedTo}${dispatchDist?` · ${dispatchDist}km away`:""}`],
                ["Report ID",      incidentId?incidentId.slice(0,10)+"…":"Saving…"],
              ].map(([k,v]) => (
